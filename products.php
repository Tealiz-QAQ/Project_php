...
$query = "SELECT id, name, price, image, motasanpham, soluong 
          FROM products" . 
          ($category ? " WHERE category = '$category'" : "") . 
          " ORDER BY (soluong = 0), id ASC 
...
<div class="container" style="margin-top:65px">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <p><strong>Số lượng trong kho:</strong> <span id="modalProductQuantity"></span></p>
    ...
    $isSoldOut = $row['soluong'] == 0; 
?>        
    <div class="col-md-3 col-6 py-2">
        <img src="images/<?php echo $row['image']; ?>"
                 alt=""
                 class="img-fluid product-image pb-1"
                 data-toggle="modal"
                 data-target="#productModal"
                 data-name="<?php echo $row['name']; ?>"
                 data-price="<?php echo number_format($row['price'], 0, ',', '.'); ?>₫"
                 data-description="<?php echo $row['motasanpham']; ?>"
                 data-quantity="<?php echo $row['soluong']; ?>"
                 data-image="images/<?php echo $row['image']; ?>">

        <div class="card-body">
            <h5 class="card-title"><?php echo $row['name']; ?></h5>
            <p class="card-text text-danger">
                <?php echo $isSoldOut ? '<strong>Hết hàng</strong>' : number_format($row['price'], 0, ',', '.') . '₫'; ?> 
            </p>
        </div>
        
        <div class="figure-caption">
            <?php if (!isset($_SESSION['user_id'])) { ?>
                <p><a href="index.php#login" class="btn btn-warning text-white">Thêm vào giỏ hàng</a></p>
            <?php } else {
                if (check_if_added_to_cart($row['id'])) {
                    echo '<p><a href="#" class="btn btn-primary text-white" disabled>Đã có trong giỏ hàng</a></p>';
                } else { ?>
                    <p><a href="cart_add.php?id=<?php echo $row['id']; ?>" class="btn btn-warning text-white">Thêm vào giỏ hàng</a></p>
                <?php }
            } ?>
        </div>
    </div> <!-- ✅ Đóng đúng thẻ col -->
<?php } ?>
</div>
...
$(document).ready(function(){
    $('#productModal').on('show.bs.modal', function (event) {
        ...
        modal.find('#modalProductQuantity').text(quantity);
