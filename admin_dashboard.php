...
$query = "SELECT id, email_id, first_name, last_name, phone, address, registration_time, username, role FROM users";
...
//Truy xuat
$query_products = "SELECT id, name, price, brand, motasanpham, image, soluong FROM products";
if ($searchTerm) {
    if ($searchField == 'name') {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['product_name'])) {
    ...
    $query = "INSERT INTO products (name, price, image, brand, motasanpham, soluong) 
          VALUES ('$productName', '$productPrice', '$productImage', '$productBrand', '$productDescription', '$productQuantity')";
...
<head>
    ...
    <style>
        ...
        #user-table th:nth-child(2){width: 30%;}
        #user-table th:nth-child(3){width: 20%;}
        #user-table th:nth-child(4){width: 10%;}
        #user-table th:nth-child(5){width: 8%;}
        #user-table th:nth-child(6){width: 5%;}
        #user-table th:nth-child(7){ width: 10%;}

        #product-table th:nth-child(1) { width: 4%; }  /* ID */
        #product-table th:nth-child(2) { width: 13%; } /* Tên Sản Phẩm - nhỏ lại */
        #product-table th:nth-child(3) { width: 10%; } /* Giá */
        #product-table th:nth-child(4) { width: 8%; }  /* Số lượng */
        #product-table th:nth-child(5) { width: 10%; } /* Loại sản phẩm - nhỏ lại */
        #product-table th:nth-child(6) { width: 30%; } /* Mô tả sản phẩm - rộng hơn */
        #product-table th:nth-child(7) { width: 10%; } /* Hình ảnh */
        #product-table th:nth-child(8) { width: 16%; } /* Hành động - mới thêm */
        ...
        .popup-content {
            ...
            width: 500px;
            max-height: 80vh;   /* Chiều cao tối đa: 80% chiều cao trình duyệt */
...
<body>
    ...
    <button class="btnedit" onclick="showEditPopup('<?php echo $product['id']; ?>', '<?php echo addslashes($product['name']); ?>', '<?php echo $product['price']; ?>', '<?php echo $product['soluong']; ?>', '<?php echo addslashes($product['brand']); ?>', '<?php echo addslashes($product['motasanpham']); ?>')">
    Sửa
</button>
</td>
        ...
        <div class="popup" id="editProductPopup">
            <div class="popup-content">
                ...
                <form method="POST" enctype="multipart/form-data" action="update_product.php">
                    ...
                    <div class="form-group">
                        <label for="edit_product_quantity">Số lượng:</label>
    ...
    <script>
        ...
        function showEditPopup(productId, productName, productPrice, productQuantity, productBrand, productDescription) {
